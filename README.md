...
SkedPal supports to fill time maps based on calendar events. Downside of this solution is that the required format `Event [Timemap Name]` is cluttering the calendar and time-consuming to write and the events are not presented anymore within SkedPal. It would be better to have the event sidelined
